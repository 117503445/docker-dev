...
但是，为什么我们要进一步将IDE（Code Server）也放入容器中呢？为什么不单纯使用VSCode Remote/Dev Containers等远程开发功能？

这是因为，随着VSCode及其扩展的不断更新，它们可能与项目所需的特定语言版本产生不兼容。例如，如果项目基于Go 1.17版本开发，而VSCode及其相关扩展持续更新，最终可能导致这些扩展与项目需求不符，从而引发兼容性问题。

此外，将IDE集成到容器中可以极大简化开发者的工作流程。只需一条命令即可启动完整的开发环境，并通过浏览器访问，无需担心本地VSCode的安装、扩展安装及个性化设置等问题。这种方法不仅能增强团队内开发环境的一致性，还能提高工作效率。

### 统一开发工具

为了进一步优化开发流程，可以考虑替换一些常用的独立应用。比如，在API调用方面，使用`humao.rest-client`扩展代替Postman。这种做法不仅简化了每个开发者的配置流程，还鼓励以文本文件形式保存API调用定义，便于Git等版本控制系统的管理。这样做有助于实现开发过程的标准化，提升团队协作效率。

### 适用范围和局限性

如本文末的例子所述，容器化开发环境可以搞定最基础的前后端开发，也能支持 CUDA、Docker in Docker 等高级特性。此外，容器内还能支持很多复杂的使用方法，比如 [uni-drop e2e-test](https://github.com/117503445/uni-drop/blob/master/docs/develop.md#e2e-test) 在容器中运行前后端、数据库、浏览器，并使用 `playwright` 完成前端的 e2e 测试。在 [nixos-playground](https://github.com/117503445/nixos-playground) 中，可以用 QEMU 运行虚拟机，并通过修改容器内网络拓扑，模拟出 Homelab 场景中的路由器和终端设备，从而允许调试路由器操作系统的 NAT 是否正常。

说明容器化开发环境必然受到容器的限制，比如嵌入式开发、windows 开发就不太适合

### 总结

综上所述，通过将开发环境容器化并将IDE集成其中，不仅可以解决传统开发环境搭建中的痛点，还能有效提升团队内部的一致性和开发效率。这一策略代表了一种更加逻辑严密且实用的方法来应对现代软件开发的复杂需求。
